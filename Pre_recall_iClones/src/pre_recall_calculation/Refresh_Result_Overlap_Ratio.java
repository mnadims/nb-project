 ...
 * @author MD NADIM
 */
public class Refresh_Result_Overlap_Ratio {
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            for(i=1000;i<=1000;i++){
                flg=0;
                query="SELECT * FROM `nicad5_result` WHERE `id` NOT IN (SELECT `id` FROM `nicad5_result_rr`) ORDER BY `version`, `CloneClass`, `file_name`, `startline`, `endline`";
                System.out.println(query);
                ...
                while(rs.next()){ 
                    ...
                    if(nxt_filename.equals(cur_filename) & nxt_class==cur_class && ((e2>=b1 && e2<=e1) || (b2>=b1 && b2<=e1) || (b2<b1 && e2>e1))){
                        if(b2>=b1 && e2<=e1){
                            overlapped=e2-b2+1;
                            System.out.println("Overlapped: "+e2+" - "+b2+"+ 1 :"+overlapped);
                            total=e1-b1+1;
                            System.out.println("Total: "+e1+" - "+b1+"+ 1 :"+total);
                        }
                        else if(e2>=b1 && e2<=e1){                           
                            overlapped=e2-b1+1;
                            ...
                            overlapped=e1-b1+1;
                            System.out.println("Overlapped: "+e1+" - "+b1+"+ 1 :"+overlapped);
                            total=e2-b2+1;
                            System.out.println("Total: "+e2+" - "+b2+"+ 1 :"+total);
                        }
                        else{
