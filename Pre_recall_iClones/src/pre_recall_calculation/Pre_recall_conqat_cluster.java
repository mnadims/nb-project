...
public class Pre_recall_conqat_cluster {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            cleaned_nicad = new File("data_files/jedit_changes.txt");
            ...
            while ((st2 = br2.readLine()) != null) {
                ...
                if(pr_version!=Integer.parseInt(splited1[0])){
                    qr_change_count="SELECT COUNT(*) AS num_change FROM `change_info` WHERE `version`='"+splited1[0]+"' AND (`endline`-`startline`+1)='"+line_count+"'";
                ...
                while (rs_detected.next()) {
                    ...
                    if(!(pr_filename.equals(cur_filename) && pr_startline==cur_startline && pr_endline==cur_endline)){
                        total_clone++;
                        qr_actual="SELECT * FROM `change_info` WHERE `version`="+splited1[0]+" AND `file_name`='"+cur_filename+"' AND (`endline`-`startline`+1)='"+line_count+"'";
                ...
                qr_insert = "INSERT INTO `pre_recall_deckard_cluster` (`change_id`, `version`, `change_detect`, `change_total`, `clone_used`, `clone_pr_total`) "
