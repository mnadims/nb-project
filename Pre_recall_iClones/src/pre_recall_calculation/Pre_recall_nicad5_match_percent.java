...
public class Pre_recall_nicad5_match_percent {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            String st2, qr_detected, qr_actual,qr_change_count, qr_percent, pr_filename,cur_filename,clone_results_table,qr_insert;
            String[] splited1;
            ...
            int k, duplicate=0, pr_b, pr_e, count;
            float percent,pr_percent, common_ln, total_ln;
            ...
            cleaned_nicad = new File("data_files/ctags_changes.txt");
            clone_results_table="nicad5_result";
            ...
            while ((st2 = br2.readLine()) != null) {
                ...
                if(total_clone>0){
                    qr_percent="SELECT `CloneClass`, `file_name`, `startline`, `endline` FROM `"+clone_results_table+"` WHERE `version`="+splited1[0]+" AND `file_name`='"+splited1[1]+"' AND "
                            +" ((`endline`>="+b1+" AND `endline`<="+e1+") OR (`startline`>="+b1+" AND `startline`<="+e1+") OR "
                            +" (`startline`<"+b1+" AND `endline`>"+e1+")) ORDER BY `file_name`, `startline`, `endline`";

                    ResultSet rs_percent=stm5.executeQuery(qr_percent);
                    common_ln=0;
                    total_ln=0;
                    pr_percent=0;
                    count=0;
                    percent=0;
                    while (rs_percent.next()) { 
                        pr_b=rs_percent.getInt("startline");
                        pr_e=rs_percent.getInt("endline");
                        if(pr_e>=b1 && pr_e<=e1){
                            common_ln=pr_e-b1+1;
                            
                        }
                        else if(pr_b>=b1 && pr_b<=e1){
                            common_ln=pr_b-b1+1;
                            
                        }
                        else if(pr_b<b1 && pr_e>e1){
                            common_ln=e1-b1+1;
                        }
                        
                        total_ln=pr_e-pr_b+1;
                        
                        percent=(common_ln/total_ln);
                        if(percent>pr_percent)
                            pr_percent=percent;
                    }
                    //percent=percent/count;
                    System.out.println(pr_percent);
                    
                }
                else pr_percent=0;
                
                qr_insert = "INSERT INTO `pre_recall_nicad5` (`change_id`, `version`, `change_detect`, `change_total`, `clone_used`, `clone_pr_total`, `match_percent`) "
                        ...
                        +"'"+splited1[0]+"', '"+chng_detect+"', '"+(total_change-1)+"', '"+used_clone+"', '"+total_clone+"' , '"+pr_percent+"')";
